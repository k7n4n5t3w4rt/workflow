...
function getUnit(val) {
  var split = /[+-]?\d*\.?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?(%|px|pt|em|rem|in|cm|mm|ex|ch|pc|vw|vh|vmin|vmax|deg|rad|turn)?$/.exec(
    val,
  );
  if (split) {
    return split[1];
...
function getPathProgress(path, progress, isPathTargetInsideSVG) {
  ...
  var p0 = point(-1);
  var p1 = point(+1);
  var xCm = isPathTargetInsideSVG ? 1 : svg.w / svg.vW;
  var yCm = isPathTargetInsideSVG ? 1 : svg.h / svg.vH;
  switch (path.property) {
    case "x":
      return (p.x - svg.x) * xCm;
    case "y":
      return (p.y - svg.y) * yCm;
    case "angle":
