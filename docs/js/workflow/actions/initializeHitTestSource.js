...
export default async () /*: Promise<void> */ => {
  ...
  sceneData.reticleStuff.hitTestSource = await session.requestHitTestSource({
    space: viewerSpace,
  });
  ...
  sceneData.reticleStuff.localSpace = await session.requestReferenceSpace(
    "local",
  );

  // set this to true so we don't request another hit source for the rest of the session
  sceneData.reticleStuff.hitTestSourceInitialized = true;

  // In case we close the AR session by hitting the button "End AR"
  session.addEventListener("end", () => {
    sceneData.reticleStuff.hitTestSourceInitialized = false;
    sceneData.reticleStuff.hitTestSource = null;
  });
};
